...
/// when Notched isn't running. See docs/claude-code-integration-notes.md §3
...
enum HookInstaller {
  ...
  private static var appSupportDirectory: URL {
    ...
    return base.appendingPathComponent("Notched", isDirectory: true)
  }
  ...
  private static var pidFileURL: URL { appSupportDirectory.appendingPathComponent("notched.pid") }
  ...
  private static var settingsBackupURL: URL {
    FileManager.default.homeDirectoryForCurrentUser.appendingPathComponent(
      ".claude/settings.json.notched-backup")
  }
  ...
  /// once Notched quits, so callers don't need to uninstall on every
  ...
  /// this machine while Notched is closed.
  ...
  private static func mergeSettings(_ mutate: (inout [String: Any]) -> Void) throws {
    ...
    let tempURL = settingsURL.appendingPathExtension("notched-tmp")
  ...
  private static func writeBridgeScript() throws {
    let script = #"""
      ...
      curl -s --connect-timeout 1 --max-time 600 \
        ...
        -H "X-Notched-Terminal: ${TERM_PROGRAM:-}" \
        -H "X-Notched-PID: ${PPID}" \
        --data-binary @-
      """#
